...
/**
	...
	当IO作为外设的(AD,串口,定时器)等特定功能引脚时，就需要开启外设.
	这里还需要开启APB2外设上的复用时钟AFIO,同时IO口才用的是复用输出!
...
{  	
	...
	选择PWM模式:
		PWM1:
			向上计数,CNT<CCR为有效电平,否则无效;向下计数CNT>CCR为有效
		PWM2??:
			与PWM1模式相反
	*/
